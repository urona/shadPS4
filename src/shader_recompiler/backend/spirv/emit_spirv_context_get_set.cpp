...
Id EmitReadConstBuffer(EmitContext& ctx, u32 handle, Id index) {
    ...
    if (Sirit::ValidId(spv_buffer.offset_dwords)) {
        index = ctx.OpIAdd(ctx.U32[1], index, spv_buffer.offset_dwords);
    }
    const auto [id, pointer_type] = spv_buffer[BufferAlias::U32];
...
Id EmitGetAttribute(EmitContext& ctx, IR::Attribute attr, u32 comp, Id index) {
    ...
    if (IR::IsParam(attr)) {
        ...
        Id result;
        if (param.is_loaded) {
            // Attribute is either default or manually interpolated.
            // The id points to an already loaded vector.
