    ...
    for (u32 i = 0; i < buffer_bindings.size(); i++) {
        ...
        if (!buffer_id) {
            ...
            if (desc.buffer_type == Shader::BufferType::GdsBuffer) {
                ...
                const auto [vk_buffer, data, offset] = buffer_cache.ObtainScratchBuffer(ubo_size, instance.UniformMinAlignment());
                std::memcpy(data, stage.flattened_ud_buf.data(), ubo_size);
                ...
                buffer_infos.emplace_back(lds_buf->Handle(), offset, lds_size);
